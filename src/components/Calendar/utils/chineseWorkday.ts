...
const WEEKENDS_WORKDAY: Record<string, string> = {
  '2018-02-11': '补春节',
  '2018-02-24': '补春节',
  '2018-04-08': '补清明节',
  '2018-04-28': '补劳动节',
...
function formatDate(day: string) {
  ...
  return {
    date: [year, month, currentDay].join('-'),
...
function isAddtionalWorkday(day: string) {
  const fd = formatDate(day)
  return !!WEEKENDS_WORKDAY[fd.date]
}

function getFestival(day: string) {
  const fd = formatDate(day)
  if (WEEKENDS_WORKDAY[fd.date]) {
    return WEEKENDS_WORKDAY[fd.date]
  }
...
export { isHoliday, isAddtionalWorkday, getFestival }
